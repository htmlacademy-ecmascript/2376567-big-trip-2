...
function createEventTemplate(event, destination, offer) {
  const { basePrice, type, favorite, dateFrom, dateTo, offersId } = event; // Добавлено offersId в деструктуризацию
  const { offers: typeOffers } = offer || {}; // Переименовано для ясности
  const { name = '' } = destination || {};

  // Изменено: Фильтрация предложений по offersId
  ...
  // Изменено: Создание шаблона предложений только для выбранных
  const createOffersTemplate = (offersArray) => offersArray.map((itemOffer) =>
    `
      <li class="event__offer">
        <span class="event__offer-title">${itemOffer.title}</span>
  ...
  return (`
    <div class="event">
      ...
      <h4 class="visually-hidden">Offers:</h4>
      ${selectedOffers.length > 0 ? // Изменено: Условный рендеринг
      `<ul class="event__selected-offers">

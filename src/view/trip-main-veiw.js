...
export default class TripMainVeiw extends AbstractView {
  ...
  _filtersClickHandler(evt) {
    evt.preventDefault();
    this._callback.filtersClick();
  }

  setFiltersClickHandler(callback) {
    this._callback.filtersClick = callback;
    this.element.querySelector('#filter-everything').addEventListener('click', this._filtersClickHandler);

    const filters = this.element.querySelectorAll('.trip-filters__filter-input');

    filters.forEach((filter) => filter.addEventListener('click', this._filtersClickHandler));
  }
}
